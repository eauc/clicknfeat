angular.module('clickApp.services').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('app/views/game/game.html',
    ...
    "                         stroke-width:2px\"></marker></defs><g id=game-deploiement ng-show=\"state.game.layers | gameLayers:'isDisplayed':'d'\" ng-include=\"'app/components/game/deploiement/deploiement.html'\"></g></svg></div></div><div id=menu><button id=menu-toggle class=\"btn btn-default\" title=\"Toggle menu\"><span class=\"glyphicon glyphicon-chevron-left\"></span> <span class=\"glyphicon glyphicon-chevron-right\"></span></button><div id=menu-content><p><strong>{{state.game | game:'description'}}</strong></p><ul class=\"nav nav-tabs\"><li class=\"{{app.stateIs('game.main') ? 'active' : ''}}\" ng-click=\"app.goToState('^.main')\"><a class=\"{{ game.hints.go_to_main ? 'go-to-hint' : ''}}\">Main</a></li><li class=\"{{app.stateIs('game.setup') ? 'active' : ''}}\" ng-click=\"app.goToState('^.setup')\"><a>Setup</a></li><li class=\"{{app.stateIs('game.log') ? 'active' : ''}}\" ng-click=\"app.goToState('^.log')\"><a>Log</a></li><li ng-click=\"app.goToState('lounge')\"><a>Lounge</a></li></ul><div ui-view id=menu-view class=\"{{game.menuDisabled() ? 'disabled' : ''}}\"></div></div></div></div>"
  );
