angular.module('clickApp.services').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('app/components/game/dice_box/dice_box.html',
    "<div><div class=btn-group><button class=\"btn btn-default\" title=\"{{game.action_bindings['roll1D6'] || ''}}\" ng-click=\"dice_box.doRollDice(6, 1)\">1D6</button> <button class=\"btn btn-default\" title=\"{{game.action_bindings['roll2D6'] || ''}}\" ng-click=\"dice_box.doRollDice(6, 2)\">2D6</button> <button class=\"btn btn-default\" title=\"{{game.action_bindings['roll3D6'] || ''}}\" ng-click=\"dice_box.doRollDice(6, 3)\">3D6</button> <button class=\"btn btn-default\" title=\"{{game.action_bindings['roll4D6'] || ''}}\" ng-click=\"dice_box.doRollDice(6, 4)\">4D6</button> <button class=\"btn btn-default\" title=\"{{game.action_bindings['roll5D6'] || ''}}\" ng-click=\"dice_box.doRollDice(6, 5)\">5D6</button> <button class=\"btn btn-default\" title=\"{{game.action_bindings['roll1D3'] || ''}}\" ng-click=\"dice_box.doRollDice(3, 1)\">1D3</button></div><div class=dice-list><div ng-repeat=\"(i,die) in dice_box.dice.slice(-10).reverse() track by die.stamp\"><span class=text-primary>[{{::die.user}}]</span> {{::die.desc}}</div></div></div>"
  );
  ...
  $templateCache.put('app/views/game/game.html',
    ...
    "                         stroke-width:2px\"></marker><clippath id=los-clip><polygon click-game-los-clip></polygon></clippath></defs><g id=game-board ng-show=\"state.game.layers | gameLayers:'isDisplayed':'b'\"><image id=board-preview x=0 y=0 width=480 height=480 xlink:href=\"{{state.game.board.preview || ''}}\"></image><image id=board-view x=0 y=0 width=480 height=480 xlink:href=\"{{state.game.board.img || ''}}\"></image></g><g id=game-terrains-locked ng-show=\"state.game.layers | gameLayers:'isDisplayed':'b'\" click-game-terrains-list=locked></g><g id=game-terrains ng-show=\"state.game.layers | gameLayers:'isDisplayed':'b'\" click-game-terrains-list=active></g><g id=game-deploiement ng-show=\"state.game.layers | gameLayers:'isDisplayed':'d'\" ng-include=\"'app/components/game/deploiement/deploiement.html'\"></g><g id=game-scenario ng-show=\"state.game.layers | gameLayers:'isDisplayed':'s'\" ng-include=\"'app/components/game/scenario/scenario.html'\"></g><g id=game-templates-locked ng-show=\"state.game.layers | gameLayers:'isDisplayed':'t'\" click-game-templates-list=locked></g><g id=game-under-models ng-show=\"state.game.layers | gameLayers:'isDisplayed':'m'\"><g click-game-los-refresh style=\"clip-path: url(#los-clip)\"><polygon ng-repeat=\"envelope in state.game.los.computed.shadow\" click-game-los-darkness class=los-shadow></polygon><polygon ng-repeat=\"envelope in state.game.los.computed.darkness\" click-game-los-darkness class=los-darkness></polygon></g></g><g id=game-models-locked ng-show=\"state.game.layers | gameLayers:'isDisplayed':'m'\" click-game-models-list=locked></g><g id=game-models ng-show=\"state.game.layers | gameLayers:'isDisplayed':'m'\" click-game-models-list=active></g><g id=game-over-models ng-show=\"state.game.layers | gameLayers:'isDisplayed':'m'\"></g><g id=game-templates ng-show=\"state.game.layers | gameLayers:'isDisplayed':'t'\" click-game-templates-list=active></g><g id=game-ruler click-game-ruler></g><g id=game-los click-game-los></g><g id=game-create-terrains><g ng-repeat=\"(index, terrain) in state.create.terrains\" click-game-create-terrain></g></g><g id=game-create-models><g ng-repeat=\"(index, model) in state.create.models\" click-game-create-model></g></g><g id=game-create-templates><g ng-repeat=\"(index, terrain) in state.create.templates\" click-game-create-template></g></g><rect id=game-dragbox click-game-dragbox></rect></svg></div></div><div id=menu><button id=menu-toggle class=\"btn btn-default\" title=\"Toggle menu\"><span class=\"glyphicon glyphicon-chevron-left\"></span> <span class=\"glyphicon glyphicon-chevron-right\"></span></button><div id=menu-content><p><strong>{{state.game | game:'description'}}</strong><form class=form-inline ng-show=\"state.game | gameConnection:'active'\" ng-submit=game.doInvitePlayer()><div class=form-group><div class=input-group><span class=input-group-btn><button type=submit class=\"btn btn-default\">Invite Player</button></span><select class=form-control ng-options=\"u.stamp as (u.name | capitalize)\n" +
    "                                  for u in state.user.connection.users\" ng-model=game.invite_player required></select></div></div></form></p><ul class=\"nav nav-tabs\"><li class=\"{{app.stateIs('game.main') ? 'active' : ''}}\" ng-click=\"app.goToState('^.main')\"><a click-chat-hint=game hint-state=game.main>Main</a></li><li class=\"{{app.stateIs('game.model') ? 'active' : ''}}\" ng-click=\"app.goToState('^.model')\"><a>Model</a></li><li class=\"{{app.stateIs('game.setup') ? 'active' : ''}}\" ng-click=\"app.goToState('^.setup')\"><a>Setup</a></li><li class=\"{{app.stateIs('game.log') ? 'active' : ''}}\" ng-click=\"app.goToState('^.log')\"><a>Log</a></li><li class=\"{{app.stateIs('game.save') ? 'active' : ''}}\" ng-click=\"app.goToState('^.save')\"><a>Save</a></li><li class=\"{{app.stateIs('game.help') ? 'active' : ''}}\" ng-click=\"app.goToState('^.help')\"><a>Help</a></li><li ng-show=\"state.user | user:'online'\" class=\"{{app.stateIs('game.online') ? 'active' : ''}}\" ng-click=\"app.goToState('^.online')\"><a click-chat-hint=user hint-state=game.online>Online</a></li><li ng-click=\"app.goToState('lounge')\"><a>Lounge</a></li></ul><div ui-view id=menu-view class=\"{{game.menuDisabled() ? 'disabled' : ''}}\"></div></div></div><click-mail-notification type=game></click-mail-notification><click-bad-dice-notification></click-bad-dice-notification></div>"
  );


  $templateCache.put('app/views/game/help/game_help.html',
    "<div><div class=\"panel panel-info\"><div class=panel-heading>Debug dump file</div><div class=panel-body><a class=\"btn btn-default\" ng-disabled=!game_help.exports.dump.url ng-href={{game_help.exports.dump.url}} download={{game_help.exports.dump.name}}>Download Dump File</a></div></div><div class=\"panel panel-info\"><div class=panel-heading>Current Mode Bindings</div><table class=table><tr ng-repeat=\"bind in game_help.bindings_pairs track by bind[0]\"><td><button class=\"btn btn-default\" ng-click=game.doModeAction(bind[0])>{{::bind[0]}}</button></td><td>{{::bind[1]}}</td><td></td></tr></table></div></div>"
  );


  $templateCache.put('app/views/game/log/game_log.html',
    "<div><div class=\"panel panel-info\"><div class=panel-heading><strong>Command Log</strong></div><div class=panel-content><button class=\"btn btn-default\" title=\"{{game.action_bindings['commandUndoLast'] || ''}}\" ng-disabled=\"log.commands.length === 0\" ng-click=\"game.doModeAction('commandUndoLast')\">Undo Last</button> <button class=\"btn btn-default\" title=\"{{game.action_bindings['commandReplayNext'] || ''}}\" ng-disabled=\"log.undo.length === 0\" ng-click=\"game.doModeAction('commandReplayNext')\">Replay Next</button><div class=log-replay-list click-log-replay-list=log.undo><div ng-repeat=\"(i,cmd) in log.undo.slice(-20) track by cmd.stamp\"><span class=text-primary>[{{::cmd.user}}]</span> {{::cmd.type}} {{::cmd.desc}}</div></div><div class=\"log-replay-list log\" click-log-replay-list=log.undo_log><div ng-repeat=\"(i,cmd) in log.undo_log.slice(-20) track by cmd.stamp\"><span class=text-primary>[{{::cmd.user}}]</span> <span class=text-info>{{::cmd.type}} {{::cmd.desc}}</span></div></div><div class=\"log-command-list log\"><div ng-repeat=\"(i,cmd) in log.commands_log.slice(-20).reverse() track by cmd.stamp\"><span class=text-primary>[{{::cmd.user}}]</span> <span class=text-info>{{::cmd.type}} {{::cmd.desc}}</span></div></div><div class=log-command-list><div ng-repeat=\"(i,cmd) in log.commands.slice(-20).reverse() track by cmd.stamp\"><span class=text-primary>[{{::cmd.user}}]</span> {{::cmd.type}} {{::cmd.desc}}</div></div></div></div></div>"
  );
  ...
  $templateCache.put('app/views/game/model/game_model.html',
    "<div><div class=\"panel panel-info\"><div class=panel-heading><strong>Create Model</strong></div><div class=panel-content><form class=form-inline name=create-model><table><tr><th>Faction</th><td><select class=form-control ng-options=\"k as f.name for (k,f) in game_model.factions\" ng-model=game_model.faction ng-change=game_model.onFactionChange()></select></td></tr><tr><th>Section</th><td><select class=form-control ng-options=\"k as k for (k,s) in\n" +
    "                                  game_model.factions[game_model.faction].models\" ng-model=game_model.section ng-change=game_model.onSectionChange()></select></td></tr><tr><th>Entry</th><td><select class=form-control ng-options=\"k as e.name for (k,e) in\n" +
    "                                  game_model.factions[game_model.faction].models[game_model.section]\" ng-model=game_model.entry ng-change=game_model.onEntryChange()></select></td></tr><tr ng-show=game_model.factions[game_model.faction].models[game_model.section][game_model.entry].entries><th>Type</th><td><select class=form-control ng-options=\"k as k for (k,t) in\n" +
    "                                  game_model.factions[game_model.faction].models[game_model.section][game_model.entry].entries\n" +
    "                                  \" ng-model=game_model.type ng-change=game_model.onTypeChange()></select></td></tr><tr ng-show=game_model.factions[game_model.faction].models[game_model.section][game_model.entry].entries><th>Model</th><td><select class=form-control ng-options=\"\n" +
    "                                  k as m.name for (k,m) in\n" +
    "                                  game_model.factions[game_model.faction].models[game_model.section][game_model.entry].entries[game_model.type]\n" +
  ...
  $templateCache.put('app/views/lounge/lounge.html',
    "<div id=lounge class=container><div class=\"panel panel-info\"><div class=panel-heading><h4 class=panel-title>{{ state.user | user:'description' }} / {{ (state.user | user:'online') ? 'Online' : 'Offline' }}</h4></div><div class=panel-body ng-show=\"state.user | user:'online'\"><click-user-chat></click-user-chat></div><div class=panel-footer><button type=button class=\"btn btn-default\" ng-click=lounge.doUserToggleOnline()>Go {{ (state.user | user:'online') ? 'Offline' : 'Online' }}</button></div></div></div>"
  );
