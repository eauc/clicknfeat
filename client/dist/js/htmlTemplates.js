angular.module('clickApp.services').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('app/components/game/chat_box/chat_box.html',
  ...
  $templateCache.put('app/components/game/models_list/models_list.html',
  ...
  $templateCache.put('app/components/games_list/games_list.html',
    "<div ng-repeat=\"(i,g) in vm.games\" class=\"clickable {{vm.current && g.public_stamp === current ? 'bg-primary':''}} {{vm.isInSelection(i) ? 'bg-info':''}}\" ng-click=vm.doSetSelection(i)>{{::g | game:'description'}}</div>"
  );
  ...
  $templateCache.put('app/views/game/game.html',
    ...
    "                         stroke-width:2px\"></marker></defs><g id=game-board ng-show=\"state.game.layers | gameLayers:'isDisplayed':'b'\"><image id=board-preview x=0 y=0 width=480 height=480 xlink:href=\"{{state.game.board.preview || ''}}\"></image><image id=board-view x=0 y=0 width=480 height=480 xlink:href=\"{{state.game.board.img || ''}}\"></image></g><g id=game-terrains-locked ng-show=\"state.game.layers | gameLayers:'isDisplayed':'b'\" click-game-terrains-list=locked></g><g id=game-terrains ng-show=\"state.game.layers | gameLayers:'isDisplayed':'b'\" click-game-terrains-list=active></g><g id=game-deploiement ng-show=\"state.game.layers | gameLayers:'isDisplayed':'d'\" ng-include=\"'app/components/game/deploiement/deploiement.html'\"></g><g id=game-templates-locked ng-show=\"state.game.layers | gameLayers:'isDisplayed':'t'\" click-game-templates-list=locked></g><g id=game-under-models ng-show=\"state.game.layers | gameLayers:'isDisplayed':'m'\"></g><g id=game-over-models ng-show=\"state.game.layers | gameLayers:'isDisplayed':'m'\"></g><g id=game-templates ng-show=\"state.game.layers | gameLayers:'isDisplayed':'t'\" click-game-templates-list=active></g><g id=game-create-terrains><g ng-repeat=\"(index, terrain) in state.create.terrains\" click-game-create-terrain></g></g><g id=game-create-templates><g ng-repeat=\"(index, terrain) in state.create.templates\" click-game-create-template></g></g></svg></div></div><div id=menu><button id=menu-toggle class=\"btn btn-default\" title=\"Toggle menu\"><span class=\"glyphicon glyphicon-chevron-left\"></span> <span class=\"glyphicon glyphicon-chevron-right\"></span></button><div id=menu-content><p><strong>{{state.game | game:'description'}}</strong></p><ul class=\"nav nav-tabs\"><li class=\"{{app.stateIs('game.main') ? 'active' : ''}}\" ng-click=\"app.goToState('^.main')\"><a class=\"{{ game.hints.go_to_main ? 'go-to-hint' : ''}}\">Main</a></li><li class=\"{{app.stateIs('game.setup') ? 'active' : ''}}\" ng-click=\"app.goToState('^.setup')\"><a>Setup</a></li><li class=\"{{app.stateIs('game.log') ? 'active' : ''}}\" ng-click=\"app.goToState('^.log')\"><a>Log</a></li><li class=\"{{app.stateIs('game.save') ? 'active' : ''}}\" ng-click=\"app.goToState('^.save')\"><a>Save</a></li><li class=\"{{app.stateIs('game.help') ? 'active' : ''}}\" ng-click=\"app.goToState('^.help')\"><a>Help</a></li><li ng-click=\"app.goToState('lounge')\"><a>Lounge</a></li></ul><div ui-view id=menu-view class=\"{{game.menuDisabled() ? 'disabled' : ''}}\"></div></div></div></div>"
  );
  ...
  $templateCache.put('app/views/game/main/game_main.html',
    "<div><div class=\"panel panel-info\"><div class=panel-heading>Tools</div><div class=panel-content><div class=btn-group><button class=\"btn btn-default\" ng-click=\"game_main.doCreateTemplate('aoe')\">New<br>AoE</button> <button class=\"btn btn-default\" ng-click=\"game_main.doCreateTemplate('spray')\">New<br>Spray</button> <button class=\"btn btn-default\" ng-click=\"game_main.doCreateTemplate('wall')\">New<br>Wall</button></div></div></div><div class=\"panel panel-info\"><div class=panel-heading>Dice</div><div class=panel-content><click-game-dice-box></click-game-dice-box></div></div></div>"
  );


  $templateCache.put('app/views/game/model/model.html',
  ...
  $templateCache.put('app/views/game/online/online.html',
  ...
  $templateCache.put('app/views/lounge/lounge.html',
    "<div id=lounge class=container><div class=\"panel panel-info\"><div class=panel-heading><h4 class=panel-title>{{ state.user | user:'description' }} / {{ (state.user | user:'online') ? 'Online' : 'Offline' }}</h4></div></div><div class=\"panel panel-info\"><div class=panel-heading><h4 class=panel-title>Local games</h4></div><div class=\"game-list panel-body\"><click-games-list games=state.local_games selection=lounge.local_games_selection></click-games-list></div><div class=panel-footer><form><div class=btn-group><button type=button class=\"btn btn-default\" ng-click=lounge.doCreateLocalGame()>New Game</button> <button type=button class=\"btn btn-default\" ng-click=lounge.doLoadLocalGame() ng-disabled=lounge.localGamesIsEmpty()>Load Game</button> <button type=button class=\"btn btn-default\" ng-click=lounge.doDeleteLocalGame(index) ng-disabled=lounge.localGamesIsEmpty()>Delete Game</button></div><div class=form-group><label for=import-file>Import game file :</label><input class=form-control id=import-file type=file click-file=\"lounge.doOpenLocalGameFile(file)\"></div></form></div></div></div>"
  );
