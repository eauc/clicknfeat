...
angular.module('clickApp.directives').directive('clickGameMap', ['$window', 'gameMap', function ($window, gameMapService) {
    ...
    link: function link(scope, element /*, attrs*/) {
      ...
      var mouseEvents = (function () {
        ...
        function mouseDownMap(event) {
          ...
          gameMapService.findEventTarget(scope.game, event).then(function onFindEventTarget(target) {
            drag = {
              active: false,
              start: start,
              target: target,
        ...
        function dragMap(event) {
          ...
          scope.$emit(emit + drag.target.type, { target: drag.target.target,
            start: drag.start,
            now: drag.now
          }, event);
        }
        function mouseLeaveMap(event) {
          ...
          if (drag.active) {
            drag.active = false;
            scope.$emit('dragEnd' + drag.target.type, { target: drag.target.target,
              start: drag.start,
              now: drag.now
            }, event);
          }
      ...
      var flipMap = (function () {
        ...
        return function flipMap() {
          deploiement_labels = R.defaultTo(document.querySelector('#deploiement-labels'), deploiement_labels);
