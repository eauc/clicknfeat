...
angular.module('clickApp.directives').controller('userConnectionCtrl', ['$scope', 'pubSub', 'userConnection', function userConnectionCtrl($scope, pubSubService, userConnectionService) {
  ...
  $scope.doSetToChat = function (chat) {
    $scope.chat.to = R.pipe(R.prop('to'), R.append(R.prop('from', chat)), R.reject(R.equals($scope.user.state.stamp)), R.uniq)(chat);
  };
  ...
  $scope.doSendChatMessageToAll = function () {
    ...
    return userConnectionService.sendChat(to, $scope.chat.msg, $scope.user).then(function () {
      $scope.chat.msg = '';
      $scope.$digest();
    });
  };
