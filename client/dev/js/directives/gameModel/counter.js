...
angular.module('clickApp.directives').factory('clickGameModelCounter', ['labelElement', 'model', function (labelElementService, modelService) {
  ...
  return {
    ...
    update: function clickGameModelCounterUpdate(map_flipped, zoom_factor, info, model, img, element) {
      ...
      var counter_text = modelService.isCounterDisplayed('c', model) ? model.state.c : '';
      var counter_center = computeCounterCenter(model);
      labelElementService.update(map_flipped, zoom_factor, counter_center.flip, counter_center.text, counter_text, counter);

      var souls_text = modelService.isCounterDisplayed('s', model) ? model.state.s + '' : '';
      ...
      labelElementService.update(map_flipped, zoom_factor, souls_center.flip, souls_center.text, souls_text, souls_label);
    }
  };
}]);
