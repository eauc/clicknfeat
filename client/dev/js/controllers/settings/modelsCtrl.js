...
angular.module('clickApp.controllers').controller('settingsModelsCtrl', ['$scope', function ($scope) {
  console.log('init settingsModelsCtrl');

  $scope.hasDesc = function (faction) {
  ...
  $scope.onStateChangeEvent('Factions.loadDescFile', function (event, result) {
    $scope.result = result;
    $scope.$digest();
  }, $scope);
