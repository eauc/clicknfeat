...
angular.module('clickApp.controllers').controller('userCtrl', ['$scope', '$state', 'user', function ($scope, $state, userService) {
  ...
  $scope.doSaveUser = function doSaveUser() {
    ...
    return R.pipeP(userService.checkOnline, $scope.setUser, function () {
      $state.go('lounge');
    })($scope.user);
  };
}]);
