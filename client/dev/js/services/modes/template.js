...
angular.module('clickApp.services').factory('templateMode', ['modes', 'settings', 'defaultMode', 'template', 'game', 'gameTemplates', 'gameTemplateSelection', function templateModeServiceFactory(modesService, settingsService, defaultModeService, templateService, gameService, gameTemplatesService, gameTemplateSelectionService) {
  ...
  template_actions.delete = function templateDelete(scope) {
    ...
    return gameService.executeCommand('deleteTemplates', stamps, scope, scope.game);
  };
  template_actions.toggleLock = function templateLock(scope) {
    var stamps = gameTemplateSelectionService.get('local', scope.game.template_selection);
    return gameTemplatesService.findStamp(stamps[0], scope.game.templates).then(function (template) {
  ...
  R.forEach(function (move) {
    template_actions[move[0]] = function templateMove(scope) {
      var stamps = gameTemplateSelectionService.get('local', scope.game.template_selection);
      return gameService.executeCommand('onTemplates', move[0], false, stamps, scope, scope.game);
    };
  ...
  (function () {
    ...
    template_actions.dragStartTemplate = function templateDragStartTemplate(scope, event) {
      ...
      scope.game.template_selection = gameTemplateSelectionService.set('local', [event.target.state.stamp], scope, scope.game.template_selection);
    };
    ...
    template_actions.dragEndTemplate = function templateDragEndTemplate(scope, event) {
      ...
      return gameService.executeCommand('onTemplates', 'setPosition', end_state, [event.target.state.stamp], scope, scope.game);
    };
  })();
