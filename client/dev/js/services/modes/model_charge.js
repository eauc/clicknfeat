...
angular.module('clickApp.services').factory('modelChargeMode', ['modes', 'settings', 'modelsMode', 'modelBaseMode', 'model', 'game', 'gameModels', 'gameModelSelection', function modelChargeModeServiceFactory(modesService, settingsService, modelsModeService, modelBaseModeService, modelService, gameService, gameModelsService, gameModelSelectionService) {
  ...
  charge_actions.endCharge = function modelEndCharge(scope) {
    ...
    return R.pipeP(function () {
      return gameService.executeCommand('onModels', 'endCharge', stamps, scope, scope.game);
    }, function () {
  ...
  charge_actions.setTargetModel = function modelSetTargetModel(scope, event) {
    ...
    return R.pipeP(gameModelsService.findStamp$(stamps[0]), function (model) {
      return R.pipeP(function () {
        ...
        return gameService.executeCommand('onModels', 'setChargeTarget', scope.factions, set_target, stamps, scope, scope.game);
      })();
    })(scope.game.models);
  };
  ...
  R.forEach(function (move) {
    charge_actions[move[0]] = buildChargeMove(move[0], move[2], false, move[0]);
    charge_actions[move[0] + 'Small'] = buildChargeMove(move[0], move[2], true, move[0] + 'Small');
  }, moves);
  function buildChargeMove(move, flip_move, small) {
    return function modelDoChargeMove(scope) {
      ...
      return R.pipeP(gameModelsService.findStamp$(stamps[0]), function (model) {
        ...
        return R.exists(target_stamp) ? gameModelsService.findStamp(target_stamp, scope.game.models) : null;
      }, function (target_model) {
        return gameService.executeCommand('onModels', _move + 'Charge', scope.factions, target_model, small, stamps, scope, scope.game);
      })(scope.game.models);
    };
  }
