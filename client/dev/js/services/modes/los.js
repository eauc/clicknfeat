...
angular.module('clickApp.services').factory('losMode', ['modes', 'settings', 'commonMode', 'game', 'gameLos',
// 'model',
// 'gameModels',
// 'gameModelSelection',
function losModeServiceFactory(modesService, settingsService, commonModeService, gameService, gameLosService
// modelService,
// gameModelsService,
// gameModelSelectionService
  ...
  los_actions.dragStartMap = function losDragStartMap(scope, drag) {
    scope.game.los = gameLosService.setLocal(drag.start, drag.now, scope, scope.game.los);
  };
  los_actions.dragMap = function losDragMap(scope, drag) {
    scope.game.los = gameLosService.setLocal(drag.start, drag.now, scope, scope.game.los);
  };
  los_actions.dragEndMap = function losDragEndMap(scope, drag) {
    return gameService.executeCommand('setLos', 'setRemote', drag.start, drag.now, scope, scope.game);
  };
