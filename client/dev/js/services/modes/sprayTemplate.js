...
angular.module('clickApp.services').factory('sprayTemplateMode', ['modes', 'settings', 'templateMode', 'sprayTemplate', 'game', 'gameTemplates', 'gameTemplateSelection', 'gameModels', function sprayTemplateModeServiceFactory(modesService, settingsService, templateModeService, sprayTemplateService, gameService, gameTemplatesService, gameTemplateSelectionService, gameModelsService) {
  ...
  template_actions.spraySize6 = function spraySize6(scope) {
    ...
    return gameService.executeCommand('onTemplates', 'setSize', 6, stamps, scope, scope.game);
  };
  template_actions.spraySize8 = function spraySize8(scope) {
    var stamps = gameTemplateSelectionService.get('local', scope.game.template_selection);
    return gameService.executeCommand('onTemplates', 'setSize', 8, stamps, scope, scope.game);
  };
  template_actions.spraySize10 = function spraySize10(scope) {
    var stamps = gameTemplateSelectionService.get('local', scope.game.template_selection);
    return gameService.executeCommand('onTemplates', 'setSize', 10, stamps, scope, scope.game);
  };
  template_actions.setOriginModel = function spraySetOriginModel(scope, event) {
    var stamps = gameTemplateSelectionService.get('local', scope.game.template_selection);
    return gameService.executeCommand('onTemplates', 'setOrigin', scope.factions, event['click#'].target, stamps, scope, scope.game);
  };
  template_actions.setTargetModel = function spraySetTargetModel(scope, event) {
    ...
    return R.pipeP(gameTemplatesService.findStamp$(stamps[0]), sprayTemplateService.origin, function (origin) {
      ...
      return gameService.executeCommand('onTemplates', 'setTarget', scope.factions, origin_model, event['click#'].target, stamps, scope, scope.game);
  ...
  function buildTemplateMove(move, small) {
    return function templateMove(scope) {
      ...
      return R.pipeP(function () {
        ...
        return gameService.executeCommand('onTemplates', move[0], scope.factions, origin_model, small, stamps, scope, scope.game);
      })();
    };
  }
