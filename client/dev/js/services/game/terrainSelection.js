...
angular.module('clickApp.services').factory('gameTerrainSelection', [function gameTerrainSelectionServiceFactory() {
  var gameTerrainSelectionService = {
    ...
    checkMode: function terrainSelectionCheckMode(scope, selection) {
      return R.pipePromise(gameTerrainSelectionService.get$('local'), R.head, function (stamp) {
        if (R.isNil(stamp)) {
    ...
    clear: function terrainSelectionClear(where, scope, selection) {
      var previous = R.prop(where, selection);
      return gameTerrainSelectionService.removeFrom(where, previous, scope, selection);
    }
  };
