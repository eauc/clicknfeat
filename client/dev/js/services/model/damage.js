...
angular.module('clickApp.services').factory('modelDamage', ['gameFactions', function modelDamageServiceFactory(gameFactionsService) {
  return function () /*modelService*/{
    var modelDamageService = {
      resetDamage: function modelResetDamage(model) {
        model.state.dmg = R.pipe(R.keys, R.reduce(function (mem, key) {
      ...
      setWarriorDamage: function modelSetWarriorDamage(factions, i, model) {
        return R.pipeP(gameFactionsService.getModelInfo$(model.state.info), function (info) {
          ...
          model.state.dmg = R.pipe(R.assoc('n', value), R.assoc('t', value))(model.state.dmg);
        })(factions);
      },
      ...
      setGridColDamage: function modelSetGridColDamage(factions, col, model) {
        return R.pipeP(gameFactionsService.getModelInfo$(model.state.info), function (info) {
          var full = R.pipe(R.addIndex(R.filter)(function (val, line) {
            return R.exists(info.damage[col][line]);
          }), R.reject(R.equals(1)), R.isEmpty)(model.state.dmg[col]);
          var value = full ? 0 : 1;
    ...
    function computeTotalGridDamage(damage) {
      return R.pipe(R.keys, R.reject(R.equals('t')), R.reject(R.equals('f')), R.reject(R.equals('n')), R.reduce(function (mem, col) {
