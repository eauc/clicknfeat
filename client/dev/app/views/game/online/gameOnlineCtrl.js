...
(function () {
  ...
  gameOnlineCtrl.$inject = ['$scope', 'user'];
  function gameOnlineCtrl($scope, userModel) {
    var vm = this;
    ...
    function activate() {
      ...
      $scope.state.user_ready.then(checkUserOnline);
    }
    ...
    function doLoadOnlineGame() {
      R.thread($scope.state)(R.pathOr([], ['user', 'connection', 'games']), R.nth(vm.games_selection.list[0]), R.defaultTo({}), R.propOr('null', 'public_stamp'), function (id) {
