...
angular.module('clickApp.controllers').controller('gameCtrl', ['$scope', '$stateParams', 'modes', function ($scope, $stateParams, modesService) {
  ...
  $scope.onStateChangeEvent('Game.load.error', function () {
    $scope.goToState('lounge');
  }, $scope);

  $scope.hints = {};
  $scope.onStateChangeEvent('Game.chat', function (event, msg) {
    if (msg.from === $scope.user.state.name) return;

    $scope.hints.go_to_main = !$scope.stateIs('game.main');
    $scope.$digest();
  }, $scope);
  $scope.onStateChangeEvent('User.chat', function (event, msg) {
  ...
  function updateCurrentModeBindings() {
    $scope.action_bindings = R.pipe(R.path(['state', 'modes']), modesService.currentModeBindings, R.clone)($scope);
    $scope.action_buttons = R.pipe(R.path(['state', 'modes']), modesService.currentModeButtons, R.clone)($scope);
    $scope.$digest();
  }
  ...
  $scope.currentModeName = function () {
    return R.pipe(R.pathOr({}, ['state', 'modes']), modesService.currentModeName)($scope);
  };
  ...
  $scope.doActionButton = function (_ref) {
