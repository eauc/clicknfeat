...
(function () {
  ...
  gameCreateTemplateDirectiveFactory.$inject = ['gameMap'];
  function gameCreateTemplateDirectiveFactory(gameMapService) {
    ...
    function link(scope, parent) {
      ...
      var template = R.nth(scope.index, state.create.templates);
      var type = template.type;

      var element = templates[type].create(document, svgNS, parent[0]);
      var is_flipped = gameMapService.isFlipped(map);
      templates[type].setPosition(state.create.base, is_flipped, template, element);
