...
(function () {
  ...
  function deleteElementCommandModelFactory() {
    return function buildDeleteElementCommandModel(type, elementModel, gameElementsModel, gameElementSelectionModel) {
      ...
      function deleteElementUndoP(ctxt, state, game) {
        return R.threadP(ctxt)(R.prop(type + 's'), R.map(tryToCreateElement), R.promiseAll, R.reject(R.isNil), R.rejectIf(R.isEmpty, 'No valid ' + type + ' definition'), onNewCreatedElements);
      ...
      function onDeletedStates(state, game, states) {
        ...
        return R.thread(game)(removeFromGameElements, removeFromGameElementSelection, emitCreateEvent$(state));
