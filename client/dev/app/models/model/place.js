  ...
  return function (MOVES, modelService) {
    var modelPlaceService = {
      startPlace: function modelStartPlace(model) {
        ...
        return R.assocPath(['state', 'pla'], { s: R.pick(['x', 'y', 'r'], model.state) }, model);
      },
      ...
      setPlaceOrigin: function modelSetPlaceOrigin(factions, other, model) {
        ...
        model = R.pipe(R.over(R.lensProp('state'), pointService.rotateAroundTo$(direction, model.state.pla.s)), R.assocPath(['state', 'pla', 's', 'r'], direction))(model);
      ...
      rotateLeftPlace: function modelRotateLeftPlace(factions, small, model) {
        ...
        model = R.pipe(R.over(R.lensProp('state'), pointService.rotateLeftAround$(angle, model.state.pla.s)), R.over(R.lensPath(['state', 'pla', 's', 'r']), R.subtract(R.__, angle)))(model);
        return modelService.checkState(factions, null, model);
      },
      rotateRightPlace: function modelRotateRightPlace(factions, small, model) {
        ...
        var angle = MOVES[small ? 'RotateChargeSmall' : 'RotateCharge'];
        model = R.pipe(R.over(R.lensProp('state'), pointService.rotateRightAround$(angle, model.state.pla.s)), R.over(R.lensPath(['state', 'pla', 's', 'r']), R.add(angle)))(model);
        return modelService.checkState(factions, null, model);
      },
