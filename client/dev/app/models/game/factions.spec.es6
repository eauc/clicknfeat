describe('gameFactionsModel', function() {
  beforeEach(inject([
    'gameFactions',
    function(gameFactionsModel) {
      this.gameFactionsModel = gameFactionsModel;
    }
  ]));
  ...
  context('buildModelsList(<list>, <user>, <references>)', function() {
    ...
    it('should init base object empty list', function() {
      expect(this.context.base).toEqual({
        x: 240, y: 240, r: 0
      });
    });
    ...
    example(function(e) {
      beforeEach(function() {
        R.forEach((key) => {
          e.references[key].regexp = this.gameFactionsModel
            .buildReferenceRegexp(key);
        }, R.keys(e.references));
      });

      context(e.desc, function() {
