...
self.spyOnService = function () {
  return function spyOnService(name) {
    ...
    angular.mock.inject([name, function (service) {
      _service = service;
      R.pipe(R.always(service), R.keys, R.filter(R.compose(R.equals('Function'), R.type, R.flip(R.prop)(service))), R.reject(R.compose(R.equals('$'), R.last)), R.forEach(function (key) {
