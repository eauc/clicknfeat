...
angular.module('clickApp.services').factory('modelMode', ['modes', 'settings', 'modelsMode', 'modelBaseMode', 'model', 'game', 'gameModels', 'gameModelSelection', function modelModeServiceFactory(modesService, settingsService, modelsModeService, modelBaseModeService, modelService, gameService, gameModelsService, gameModelSelectionService) {
  ...
  model_actions.startCharge = function modelStartCharge(scope) {
    ...
    return R.pipeP(function () {
      return gameService.executeCommand$('onModels', 'startCharge', stamps, scope, scope.game);
    }, function () {
  ...
  model_actions.startPlace = function modelStartPlace(scope) {
    var stamps = gameModelSelectionService.get('local', scope.game.model_selection);
    return R.pipeP(function () {
      return gameService.executeCommand('onModels', 'startPlace', stamps, scope, scope.game);
    }, function () {
