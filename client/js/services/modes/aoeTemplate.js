...
angular.module('clickApp.services').factory('aoeTemplateMode', ['modes', 'settings', 'templateMode', 'game', 'gameTemplates', 'gameTemplateSelection', 'gameRuler', 'prompt', function aoeTemplateModeServiceFactory(modesService, settingsService, templateModeService, gameService, gameTemplatesService, gameTemplateSelectionService, gameRulerService, promptService) {
  ...
  template_actions.aoeSize3 = function aoeSize3(scope) {
    ...
    return gameService.executeCommand('onTemplates', 'setSize', 3, stamps, scope, scope.game);
  };
  template_actions.aoeSize4 = function aoeSize4(scope) {
    var stamps = gameTemplateSelectionService.get('local', scope.game.template_selection);
    return gameService.executeCommand('onTemplates', 'setSize', 4, stamps, scope, scope.game);
  };
  template_actions.aoeSize5 = function aoeSize5(scope) {
    var stamps = gameTemplateSelectionService.get('local', scope.game.template_selection);
    return gameService.executeCommand('onTemplates', 'setSize', 5, stamps, scope, scope.game);
  };
  template_actions.setTargetModel = function aoeSetTargetModel(scope, event) {
    var stamps = gameTemplateSelectionService.get('local', scope.game.template_selection);
    return gameService.executeCommand('onTemplates', 'setTarget', scope.factions, null, event['click#'].target, stamps, scope, scope.game);
  };
  template_actions.setMaxDeviation = function rulerSetMaxDeviation(scope) {
    ...
    return R.pipeP(function () {
      ...
      return gameTemplatesService.onStamps('setMaxDeviation', value, stamps, scope.game.templates);
    })();
  };
  template_actions.deviate = function aoeDeviate(scope) {
    ...
    return R.pipeP(function () {
      ...
      return gameService.executeCommand('onTemplates', 'deviate', deviation.r, deviation.d, stamps, scope, scope.game);
    })();
  };
  template_actions.setToRulerTarget = function aoeSetToRulerTarget(scope) {
    ...
    return R.pipeP(function () {
      ...
      return gameService.executeCommand('onTemplates', 'setToRuler', position, stamps, scope, scope.game);
    })();
  };
