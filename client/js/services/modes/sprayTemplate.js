...
self.sprayTemplateModeServiceFactory = function sprayTemplateModeServiceFactory(modesService,
  ...
  template_actions.spraySize6 = function spraySize6(scope) {
    ...
    gameService.executeCommand('onTemplates', 'setSize', 6, stamps, scope, scope.game);
  };
  template_actions.spraySize8 = function spraySize8(scope) {
    var stamps = gameTemplateSelectionService.get('local', scope.game.template_selection);
    gameService.executeCommand('onTemplates', 'setSize', 8, stamps, scope, scope.game);
  };
  template_actions.spraySize10 = function spraySize10(scope) {
    var stamps = gameTemplateSelectionService.get('local', scope.game.template_selection);
    gameService.executeCommand('onTemplates', 'setSize', 10, stamps, scope, scope.game);
  };
  template_actions.clickModel = function sprayClickModel(scope, event, dom_event) {
    var stamps = gameTemplateSelectionService.get('local', scope.game.template_selection);
    if(dom_event.ctrlKey) {
      gameService.executeCommand('onTemplates', 'setOrigin',
                                 scope.factions, event.target,
                                 stamps, scope,  scope.game);
      return;
    }
    else if(dom_event.shiftKey) {
      var spray = gameTemplatesService.findStamp(stamps[0], scope.game.templates);
      ...
      gameService.executeCommand('onTemplates', 'setTarget',
                                 scope.factions, origin_model, event.target,
                                 stamps, scope,  scope.game);
      return;
    }
  ...
  R.forEach(function(move) {
    template_actions[move[0]] = function templateMove(scope) {
      ...
      gameService.executeCommand('onTemplates', move[0],
                                 scope.factions, origin_model, false,
                                 stamps, scope, scope.game);
    };
    template_actions[move[0]+'Small'] = function templateMove(scope) {
      ...
      gameService.executeCommand('onTemplates', move[0],
                                 scope.factions, origin_model, true,
                                 stamps, scope, scope.game);
    };
  }, moves);
