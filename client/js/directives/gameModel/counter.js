...
angular.module('clickApp.directives').factory('clickGameModelCounter', ['labelElement', 'model', function (labelElementService, modelService) {
  ...
  return {
    ...
    update: function clickGameModelCounterUpdate(map_flipped, zoom_factor, info, model, img, el) {
      ...
      labelElementService.update(map_flipped, zoom_factor, counter_center.flip, counter_center.text, modelService.isWreckDisplayed(model) ? '' : counter_text, counter);

      var souls_text = !modelService.isWreckDisplayed(model) && modelService.isCounterDisplayed('s', model) ? model.state.s + '' : '';
